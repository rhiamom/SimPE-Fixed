...
public abstract class BaseDockPanelRenderer : BaseControlRenderer, IDisposable
{
	...
	public virtual Padding GetGripSize(DockStyle dock)
	{
		return dock switch
		{
			DockStyle.Right => new Padding(Dimension.GripSize, 0, 0, 0), 
			DockStyle.Bottom => new Padding(0, Dimension.GripSize, 0, 0), 
			DockStyle.Left => new Padding(0, 0, Dimension.GripSize, 0), 
			DockStyle.Top => new Padding(0, 0, 0, Dimension.GripSize), 
			_ => new Padding(0), 
		};
	}
	...
	public virtual Rectangle GetGripRectangle(NCPaintEventArgs e, DockStyle dock)
	{
		return dock switch
		{
			DockStyle.Right => new Rectangle(-1, -1, Dimension.GripSize + 1, e.WindowRectangle.Height + 1), 
			DockStyle.Bottom => new Rectangle(-1, -1, e.WindowRectangle.Width + 1, Dimension.GripSize + 1), 
			DockStyle.Left => new Rectangle(e.WindowRectangle.Width - Dimension.GripSize, -1, Dimension.GripSize + 1, e.WindowRectangle.Height + 1), 
			DockStyle.Top => new Rectangle(-1, e.WindowRectangle.Height - Dimension.GripSize, e.WindowRectangle.Width + 1, Dimension.GripSize + 1), 
			_ => Rectangle.Empty, 
		};
	}
	...
	public virtual Padding GetPanelBorderSize(DockContainer dc, DockPanel dp, ButtonOrientation orient)
	{
		...
		{
			ButtonOrientation.Bottom => new Padding(num, num2 + num, num, num + buttonsRectangle.Height), 
			ButtonOrientation.Right => new Padding(num, num2 + num, num + buttonsRectangle.Width, num), 
			ButtonOrientation.Top => new Padding(num, num2 + num + buttonsRectangle.Height, num, num), 
			_ => new Padding(num + buttonsRectangle.Width, num2 + num, num, num), 
		};
	}
	...
	protected virtual int GetButtonCaptionWidth(Font font, Graphics g, string caption, ButtonOrientation orient, int maxwd)
	{
		StringFormat format = ((orient != ButtonOrientation.Bottom && orient != ButtonOrientation.Top) ? new StringFormat(StringFormatFlags.DirectionVertical | StringFormatFlags.NoWrap | StringFormatFlags.NoClip) : new StringFormat(StringFormatFlags.NoWrap | StringFormatFlags.NoClip));
	...
	protected virtual string GetFittingString(Font font, Graphics g, string caption, ButtonOrientation orient, Size maxsz)
	{
		int num = maxsz.Width;
	...
	public void RenderButton(Graphics g, Rectangle r, string caption, Image img, ButtonOrientation orient, ButtonState state, bool renderbackgroundbar)
	{
		if (r.Height != 0 && r.Width != 0)
		{
			...
			StringFormat sf = ((orient != ButtonOrientation.Bottom && orient != ButtonOrientation.Top) ? new StringFormat(StringFormatFlags.DirectionVertical) : new StringFormat());
	...
	protected abstract void RenderButton(Graphics g, Rectangle r, Rectangle fullr, string caption, Image img, Color c, Color ci, Color fontc, Font f, StringFormat sf, ButtonOrientation orient, ButtonState state, bool renderbackgroundbar);
	...
	public Rectangle GetButtonsRectangle(ButtonOrientation orient, NCPaintEventArgs e, DockContainer dc)
	{
		...
		return orient switch
		{
			ButtonOrientation.Bottom => new Rectangle(0, e.WindowRectangle.Height - num, e.WindowRectangle.Width, num), 
			ButtonOrientation.Top => new Rectangle(0, 0, e.WindowRectangle.Width, num), 
			ButtonOrientation.Left => new Rectangle(0, 0, num, e.WindowRectangle.Height), 
			_ => new Rectangle(e.WindowRectangle.Width - num, 0, num, e.WindowRectangle.Height), 
		};
	}
