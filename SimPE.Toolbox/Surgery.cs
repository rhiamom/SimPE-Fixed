...
namespace SimPe.Plugin
{
    ...
    public class Surgery : System.Windows.Forms.Form
    {
        ...
        // Neighborhood package the surgery tool is working on
        private SimPe.Interfaces.Files.IPackageFile ngbh;

        // Provider registry used for lookups
        private SimPe.Interfaces.IProviderRegistry prov;

        // The selected Sim descriptor (result) that will be returned from Execute
        private SimPe.Interfaces.Files.IPackedFileDescriptor pfd;
    ...
    Interfaces.IProviderRegistry prov)
        {
            ...
            // Show “UBI” label for special hood types if present
            SimPe.Plugin.Idno idno = SimPe.Plugin.Idno.FromPackage(package);
            if (idno != null)
                this.lbUbi.Visible = (idno.Type != NeighborhoodType.Normal);

            // Store context so other methods (FillList, PlasticSurgery, etc.) can use it
            this.pfd = null;
            this.prov = prov;
            this.ngbh = package;
            ...
            // Load all Sims from the neighborhood
            ...
            try
            {
                ...
                {
                    var sdesc = new SimPe.PackedFiles.Wrapper.ExtSDesc();
            ...
            // Show the dialog as a subform
            RemoteControl.ShowSubForm(this);

            // If user picked a Sim, propagate that back out
            if (this.pfd != null)
                pfd = this.pfd;
        ...
        public Surgery()
        {
            ...
            ThemeManager.Global.AddControl(this.groupBox1);
            ThemeManager.Global.AddControl(this.groupBox2);
            ThemeManager.Global.AddControl(this.groupBox3);
        ...
        {
            // Any time the gender/age/townie checkboxes change,
            // rebuild the list based on the current filter settings.
            FillList();
        }
        ...
        void LoadSkins()
        {
            ...
            try
            {
                ...
                foreach (SimPe.Interfaces.Scenegraph.IScenegraphFileIndexItem item in items)
                {
                    ...
                    {
                        // Maintain a list of all available skin files per skintone
                        ...
                        if ((skin.GetSaveItem("override0subset").StringValue == "body" || skin.GetSaveItem("override0subset").StringValue == "top") &&
                            skin.GetSaveItem("gender").UIntegerValue == 1 &&
                            skin.GetSaveItem("age").UIntegerValue == (uint)Data.Ages.Adult)
                        {
                            ...
                            if (idr.Length > 0)
                            {
                                ...
                                // If HiddenMode is enabled, append the raw skintone key
                                if (Helper.WindowsRegistry.HiddenMode)
                                    lvi.Text += " (" + skin.GetSaveItem("skintone").StringValue + ")";
                                    ...
                                    if (pfd.Type == Data.MetaData.TXMT)
                                    {
                                        ...
                                        if (txmts.Length > 0)
                                        {
                                            ...
                                            SimPe.Interfaces.Scenegraph.IScenegraphFileIndexItem txtri =
                                                FileTable.FileIndex.FindFileByName(txtrname, Data.MetaData.TXTR, Data.MetaData.LOCAL_GROUP, true);
                                            if (txtri != null)
                                            {
                    ...
                    } // Don't need to process every item of clothing do we
                } // foreach items
            }
            finally
            {
        ...
        protected void FillList()
        {
            ...
            // Set the default skin thumb based on selected options (de-boobified)
            if (this.cbgals.Checked)
                this.iskin.Images[0] = ImageLoader.Preview(SimPe.GetImage.SheOne, this.iskin.ImageSize);
            else if (this.cbmens.Checked)
                this.iskin.Images[0] = ImageLoader.Preview(SimPe.GetImage.NoOne, this.iskin.ImageSize);
            else
                this.iskin.Images[0] = ImageLoader.Preview(SimPe.GetImage.NoOne, this.iskin.ImageSize);
            ...
            Interfaces.Files.IPackedFileDescriptor[] pfds =
                this.ngbh.FindFiles(Data.MetaData.SIM_DESCRIPTION_FILE);
            ...
            try
            {
                foreach (Interfaces.Files.IPackedFileDescriptor spfd in pfds)
                {
                    ...
                    sdesc.ProcessData(spfd, this.ngbh);
                    AddSim(sdesc);
                }
                ...
                this.llusepatient.Enabled = false;
                this.llexport.Enabled = false;
                if (this.lv.Items.Count > 0) this.lv.Items[0].Selected = true;
            }
            finally
            {
                WaitingScreen.Stop(this);
