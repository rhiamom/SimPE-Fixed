...
namespace SimPe
{
    ...
    public partial class infocheck : UserControl
	{
        ...
        private void button1_Click(object sender, System.EventArgs e)
        {
            ...
            foreach (string file in files)
            {
                if (file.Contains("Setup") || file.Contains("ASCIIart") || file.Contains("unins00")) continue;
                listing.Add(new FileDescriptor(ReleaseDir, file));
            }
        ...
        private void shouldexist(bool founde)
        {
            ...
            foreach (string file in files)
            {
                if (file.Contains("simpe.null.plugin") || file.Contains("simpe.dnaupd.plugi")) continue;
                listing.Add(new FileDescriptor(ReleaseDir + "Plugins", file));
            }
            ...
            foreach (string file in files)
            {
                if (file.Contains("Setup") || file.Contains("ASCIIart") || file.Contains("unins0")) continue;
                listing.Add(new FileDescriptor(ReleaseDir, file));
            }
