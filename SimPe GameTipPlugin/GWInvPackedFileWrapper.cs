...
namespace SimPe.Plugin
{
		...
		: AbstractWrapper, IFileWrapper, IFileWrapperSaveExtension
    {
        ...
        public ushort novels;   // Number of Novels available on the 'web', T&A and A&N only
		...
		public GWInvPackedFileWrapper() : base()
		{ }
        ...
        {
            for (int i = 0; i < 200; i++) { TipsList[i] = 0; }
            novels = 0;
            hieps = 0;
            loweps = 0;
            ...
            if (trunned != 0)
            {
                ...
                for (int i = 0; i < trunned; i++)
                {
                    ...
                    if (guide == 0xf3f5c2ac) novels += 1;
                    if (guide == 0xaef5633a)
                        {
                            if (versh > 202) reader.BaseStream.Seek(0xa, System.IO.SeekOrigin.Current); // Seasons ++
                            if (versh > 190 && versh < 203) reader.BaseStream.Seek(0x8, System.IO.SeekOrigin.Current); // OFB to Pets 
                            if (versh < 191) reader.BaseStream.Seek(0x6, System.IO.SeekOrigin.Current); // Nightlife (Base & Uni don't setup the Inventory)
                            ...
                            if (fnde == 1)
                            {
                                ...
                                if (quanty > 2) hieps = reader.ReadUInt16(); // game doesn't set game flags 2 except for T&A + A&N
                            }
                            else
                            {
                                if (quanty > 1) fnd = reader.ReadUInt16(); // move one more if any hold an extra data
                                if (quanty > 2) fnd = reader.ReadUInt16(); // check for one more just in case
                            }
                        }
                    else reader.BaseStream.Seek(0xE, System.IO.SeekOrigin.Current); // move to the end of an 8 byte token, required for Novels
               }
            }
          }
