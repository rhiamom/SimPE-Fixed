...
{
    public partial class LastEPusePackedFileUI : SimPe.Windows.Forms.WrapperBaseControl, IPackedFileUI
    {
    ...
    Label lbl;
        ...
        lastep = Wrapper.Prevep;
            ...
            if (lastep == 0)
            {
                if (vergin == 2) previep.Text = "The Sims Life Stories";
                else previep.Text = "The Sims 2 (Base Game)";
            }
            if (lastep == 1) previep.Text = "The Sims 2 University";
            if (lastep == 2) previep.Text = "The Sims 2 Nightlife";
            if (lastep == 3) previep.Text = "The Sims 2 Business";
            if (lastep == 4) previep.Text = "The Sims 2 Family Fun";
            if (lastep == 5) previep.Text = "The Sims 2 Glamour Life";
            if (lastep == 6)
            {
                if (vergin == 3) previep.Text = "The Sims Pet Stories";
                else previep.Text = "The Sims 2 Pets";
            }
            if (lastep == 7)
            {
                if (vergin == 9) previep.Text = "The Sims Castaway Stories";
                else previep.Text = "The Sims 2 Seasons";
            }
            if (lastep == 8) previep.Text = "The Sims 2 Celebration";
            if (lastep == 9) previep.Text = "The Sims 2 Fashion";
            if (lastep == 10) previep.Text = "The Sims 2 Bon Voyage";
            if (lastep == 11) previep.Text = "The Sims 2 Teen Style";
            if (lastep == 12) previep.Text = "Sims 2 StoreEdition_old";
            if (lastep == 13) previep.Text = "The Sims 2 Freetime";
        ...
        private void setChapter(Dictionary<uint, uint> dict, int chapter, string headr)
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                if (dict.ContainsKey((uint)Wrapper.vdata.GetValue(chapter, i)))
                {
                    ...
                    wy += 64;
                    if (wy > CatawayPnl.Height - 64) { wy = 53; ex += 300; }
                }
            }
        }
        ...
        {
            if (mis == 1) setChapter(Chap01, mis, "Almost Paradise");
            if (mis == 2) setChapter(Chap02, mis, "I Will Survive");
            if (mis == 3) setChapter(Chap03, mis, "Still Haven't Found What I'm Looking For");
            if (mis == 4) setChapter(Chap04, mis, "Idol Hands");
            if (mis == 5) setChapter(Chap05, mis, "The Eye That Binds");
            if (mis == 6) setChapter(Chap06, mis, "A Brand New Day");
            if (mis == 7) setChapter(Chap07, mis, "The Shady Lagoon");
            if (mis == 8) setChapter(Chap08, mis, "A Class Act");
            if (mis == 9) setChapter(Chap09, mis, "You Got Served");
            if (mis == 10) setChapter(Chap10, mis, "A Fresh Start");
            if (mis == 11) setChapter(Chap11, mis, "A Social Networking Site");
            if (mis == 12) setChapter(Chap12, mis, "A Tribal Affair");
            if (mis == 13) setChapter(Chap13, mis, "Hot Times");
            if (mis == 14) setChapter(Chap14, mis, "Full Court Press");
            if (mis == 15) setChapter(Chap15, mis, "Village People");
            if (mis == 16) setChapter(Chap16, mis, "Idol Fancy");
            if (mis == 17) setChapter(Chap17, mis, "The Cat's Meow");
            if (mis == 18) setChapter(Chap18, mis, "Spirit Quest");
            if (mis == 19) setChapter(Chap19, mis, "Luau Time");
            if (mis == 20) setChapter(Chap20, mis, "Home Is Where The Hut Is");
            if (mis == 21) setChapter(Chap21, mis, "The Tides Turn");
            if (mis == 22) setChapter(Chap22, mis, "Eruption");
            if (mis == 23) setChapter(Chap23, mis, "Let My Love Open The Door");
            if (mis == 24) setChapter(Chap24, mis, "The Power Of Love");
        }
    }
}
