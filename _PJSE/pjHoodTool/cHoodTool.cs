...
namespace pjHoodTool
{
    class cHoodTool : SimPe.Interfaces.AbstractTool, ITool, ICommandLine
    {
        ...
        internal static bool incuni = SimPe.PathProvider.Global.GetExpansion(SimPe.Expansions.University).Exists;
        internal static bool incbus = SimPe.PathProvider.Global.GetExpansion(SimPe.Expansions.Business).Exists;
        internal static bool incfre = SimPe.PathProvider.Global.GetExpansion(SimPe.Expansions.FreeTime).Exists;
        internal static bool incapa = SimPe.PathProvider.Global.GetExpansion(SimPe.Expansions.Apartments).Exists;
        ...
        void Rufio(string output, string hood, int group)
        {
            ...
            else
            {
                ...
                output = Path.Combine(Path.Combine(output, foldim), "ExportedSims" + outptype);
            }
        ...
        void AddSim(string outPath, string hood, string hoodName, StreamWriter w, ExtSDesc sdsc)
        {
            ...
            if (pkg == null)
            {
                if (SimPe.Data.MetaData.GetKnownNPC(sdsc.SimId) != "not found") desc = q(SimPe.Data.MetaData.GetKnownNPC(sdsc.SimId)) + desc + "(NPC)";
                else return;
            }
            else
            {
                ...
                else
                {
                    ...
                    catch
                    {
                        ...
                        {
                            if (incdes)
                                desc = q(SimPe.Data.MetaData.GetKnownNPC(sdsc.SimId)) + ",,(NPC)";
                            else
                                desc = q(SimPe.Data.MetaData.GetKnownNPC(sdsc.SimId)) + ",(NPC)";
                        }
                        if (desc.EndsWith(",") && incdes) desc += ","; // if the CTSS does not have three lines, will throw trying to read lastname so we need a nuver comma
                    }
                }
            }
            #endregion
            ...
            {
                if (lotfams[ltxt.LotDescription.Instance] == "" && ltxt.Type == Ltxt.LotType.Residential) lotfams[ltxt.LotDescription.Instance] = "*For Sale*";
            }
            w.WriteLine(hood +
                "," + hoodName +
                "," + hoodtype +
        ...
        public string[] Help()
        {
            return new string[] { "-rufio -out {outpath} {-hood hood} {-group group}", "Export Neighbourhood data" };
        }
