...
namespace SimPe.Plugin
{
		...
		: AbstractWrapper, IFileWrapper, IFileWrapperSaveExtension
    {
		...
		public SimmyListPackedFileWrapper() : base()
		{ }
		...
		protected override void Unserialize(System.IO.BinaryReader reader)
        {
            ...
            if (idno != null)
            {
                if (idno.Type != SimPe.Plugin.NeighborhoodType.Normal) { strung = "-INVALID FILE-\r\nData only Valid in a Primary Neighbourhood\r\n\r\n"; twine = "-INVALID FILE-\r\nData only Valid in a Primary Neighbourhood\r\n\r\n"; }
            }
                ...
                for (int i = 0; i < sluts; i++)
                {
                    ...
                    if (Helper.IsLotCatalogFile(package.FileName)) // Search by Nid, slow but accurate
                    {
                        ...
                        foreach (SimPe.Interfaces.Files.IPackedFileDescriptor pfd in files)
                        {
                            ...
                            {
                                strung += "0x" + Helper.HexString(Convert.ToInt16(slut)) + " - " + sdesc.SimName + " " + sdesc.SimFamilyName + "\r\n";
                                dided = true;
                            }
                        }
                        if (!dided)
                        {
                            strung += "0x" + Helper.HexString(Convert.ToInt16(slut)) + " - MISSING\r\n";
                            twine += "0x" + Helper.HexString(Convert.ToInt16(slut)) + " - MISSING\r\n";
                        }
                    }
                    else // search by instance, fast but not as accurate
                    {
                        SimPe.Interfaces.Files.IPackedFileDescriptor pfd = package.FindFile(Data.MetaData.SIM_DESCRIPTION_FILE, 0, 0xFFFFFFFF, slut);
                        if (pfd == null) { strung += "0x" + Helper.HexString(Convert.ToInt16(slut)) + " - MISSING\r\n"; twine += "0x" + Helper.HexString(Convert.ToInt16(slut)) + " - MISSING\r\n"; }
                        else
                        {
                            PackedFiles.Wrapper.ExtSDesc sdesc = new SimPe.PackedFiles.Wrapper.ExtSDesc();
                            sdesc.ProcessData(pfd, package);
