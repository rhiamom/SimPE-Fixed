...
namespace SimPe.Plugin
{
		...
		: AbstractWrapper, IFileWrapper, IFileWrapperSaveExtension
    {
        ...
        /// </summary>
        private ushort[] fval;
        private int sextions = 0;
        private int goodsex = 0;
		...
		protected override void Unserialize(System.IO.BinaryReader reader)
        {
            ...
            sextions = reader.ReadInt32();
            int siser = sextions * 42; // 83 bytes - 1 byte + 41 two bit bytes
            Array.Resize<ushort>(ref fval, siser);

            reader.BaseStream.Seek(17, System.IO.SeekOrigin.Begin);
            try
            {
                for (int i = 0; i < sextions; i++)
                {
                    for (int j = 0; j < 41; j++)
